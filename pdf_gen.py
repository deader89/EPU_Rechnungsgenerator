...
def s(text):
    # Ersetzt das Euro-Zeichen durch EUR und fängt andere ungültige Sonderzeichen (wie Emojis) ab, 
    # um einen FPDF-Absturz (UnicodeEncodeError) zu verhindern.
    if text is None: return ""
    return str(text).replace("€", "EUR").encode('latin-1', 'replace').decode('latin-1')

def generiere_banking_qr_pfad(betrag, rechnungs_nummer, meine_daten):
    ...
    try:
        ...
        temp_path = os.path.join(temp_dir, f"qr_{rechnungs_nummer}.png")
    except Exception:
        temp_path = f"qr_{rechnungs_nummer}.png"
...
def baue_pdf_im_ram(meine_daten, kunde_name, kunde_daten, rechnungsnummer, positionen, passwort=None):
    ...
    firma = s(meine_daten.get('firma', ''))
    strasse = s(meine_daten.get('strasse', ''))
    plz_ort = s(meine_daten.get('plz_ort', ''))
    
    header_lines = [firma, strasse, plz_ort]
    if meine_daten.get('email'): header_lines.append(s(meine_daten['email']))
    if seine_tel := meine_daten.get('tel'): header_lines.append(s(f"Tel: {seine_tel}"))
    if meine_daten.get('uid'): header_lines.append(s(f"UID: {meine_daten['uid']}"))
    ...
    for i, line in enumerate(header_lines):
        pdf.set_font("helvetica", "B" if i == 0 or line.startswith("UID:") else "", 10)
    ...
    formatiert_adresse = s(kunde_daten.get('adresse', '')).split('\n')
    empfaenger_lines = ["Empfänger:", s(kunde_name)]
    empfaenger_lines.extend(formatiert_adresse)
    if kunde_daten.get('uid'): 
        empfaenger_lines.append(s(f"UID: {kunde_daten['uid']}"))
    ...
    for idx, p in enumerate(positionen, 1):
        ...
        try: mwst_prozent = float(str(p.get("mwst", meine_daten.get("steuersatz", 20))).replace(',', '.'))
        except ValueError: mwst_prozent = 20.0
        
        netto_preis = brutto_preis / (1 + (mwst_prozent / 100.0))
    ...
    infotext_lines = []
    if Zziel:
        infotext_lines.append(s(f"Bitte überweisen Sie den Bruttobetrag innerhalb von {Zziel} Tagen auf das unten angegebene Bankkonto."))
    ...
    if skonto_p > 0:
        skonto_abzug = brutto_gesamt * (skonto_p / 100.0)
        infotext_lines.append(s(f"Skonto-Option: Bei Zahlung innerhalb von {skonto_t} Tagen gewähren wir {skonto_p:.0f}% Skonto."))
        infotext_lines.append(s(f"Sie überweisen in diesem Fall einen reduzierten Betrag von {brutto_gesamt - skonto_abzug:.2f} EUR."))
    ...
    if meine_daten.get("qr_aktiv", "Ja") == "Ja" and meine_daten.get("iban") and meine_daten.get("bic"):
        try:
            qr_pfad = generiere_banking_qr_pfad(brutto_gesamt, rechnungsnummer, meine_daten)
            y_qr = pdf.get_y()
            ...
            pdf.image(qr_pfad, x=15, y=y_qr, w=25, h=25)
    ...
    if custom_footer:
        for line in custom_footer.split('\n'):
            pdf.cell(0, 4, s(line), align="C", new_x="LMARGIN", new_y="NEXT")
    else:
    ...
    if meine_daten.get('iban', '').strip() or meine_daten.get('bic', '').strip():
        pdf.cell(0, 4, s(f"Bankverbindung: IBAN: {meine_daten.get('iban', '')} | BIC: {meine_daten.get('bic', '')}"), align="C", new_x="LMARGIN", new_y="NEXT")
...
def export_kunden_dsgvo_pdf(name, daten):
    ...
    pdf.cell(0, 6, s(f"Auskunft über gespeicherte personenbezogene Daten für: {name}"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)
    ...
    pdf.set_font("helvetica", "", 12)
    pdf.cell(0, 6, s(f"Name/Firma: {name}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, s(f"Adresse: {daten.get('adresse', 'Nicht hinterlegt')}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, s(f"UID-Nummer: {daten.get('uid', 'Nicht hinterlegt')}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, s(f"E-Mail: {daten.get('email', 'Nicht hinterlegt')}"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    
    pdf.set_font("helvetica", "B", 14)
    pdf.cell(0, 8, "Zahlungskonditionen:", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("helvetica", "", 12)
    pdf.cell(0, 6, s(f"Individuelles Zahlungsziel: {daten.get('zahlungsziel', 'Standard')}"), new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, s(f"Skonto: {daten.get('skonto_prozent', 'Standard')}% innerhalb von {daten.get('skonto_tage', 'Standard')} Tagen"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    pdf.cell(0, 6, s(f"Kunde im System angelegt am: {erstellt if erstellt else 'Unbekannt'}"), new_x="LMARGIN", new_y="NEXT")
    
    letzte_rn = daten.get('letzte_rechnung_datum', '')
    if letzte_rn:
        try: letzte_rn = datetime.fromisoformat(letzte_rn).strftime('%d.%m.%Y')
        except Exception: pass
    pdf.cell(0, 6, s(f"Letzte Rechnungsstellung: {letzte_rn if letzte_rn else 'Keine'}"), new_x="LMARGIN", new_y="NEXT")
...
def export_datenschutzerklaerung_pdf(text):
    ...
    for paragraph in text.split('\n'):
        ...
        if paragraph:
            pdf.multi_cell(0, 5, s(paragraph), new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
